...
| 896 | Memory hierarchy L0–L5 | 🟡 | `memory::MemoryTier` (a classification vocabulary only — documents carry no tier and nothing promotes/demotes across tiers yet) + `rank_memories` (Generative-Agents recency×importance×relevance; consumed by `Session::recall`, reachable via `ckos history <dir> <query…>`, see §927) |
