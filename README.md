...
./scripts/verify-quickstart.sh # run every command below against a release build
```
...
| `run [--session <dir>] [--role R \| --token T] <intent…>` | Plan + execute; `--session` persists the run and grows its knowledge graph; `--role`/`--token` select the identity §929 authorization runs as for finance/medical/legal/robotics steps. Authorization is **always on**: with neither flag you are `guest`, which those capabilities deny. `--token` authenticates via a demo identity provider (§928) carrying real ABAC attributes, unlike bare `--role`. **Caution**: the built-in planner never classifies free text into those capabilities, so neither flag has an observable effect via `run` — see `ckos workflow` | `ckos run --session ./sess "research X"` |
...
| `workflow [--role <role> \| --token <token>] <file>` | Load and execute a workflow definition file (`workflow: <name>` then `step <name>: <capability> [<- dep]` lines). The only way to author a gated finance/medical/legal/robotics step, so the only place §929 is observable: with neither flag you run as `guest` and such a step is **denied**; `--role admin` or `--token tok-admin-hq` authorizes it. `--token` carries real ABAC attributes (§928: `tok-admin-hq`, `tok-admin-restricted`, `tok-guest`) | `ckos workflow pipeline.wf` |
